...
BOOLEAN init_fuelsel( void ){
  ...
  if( xSemaphoreFuelsel != NULL ){
    xTaskCreate( prvFuelselTask, "fuelsel task", configMINIMAL_STACK_SIZE, NULL, ( tskIDLE_PRIORITY + 3 ), NULL );
    tilPrint("fuelsel initialized\r\n");
    return 1;
  } else {
    return 0;
  }
...
static void prvFuelselTask( void *pvParameters )
{
  ...
  for( ;; ){
    if( xSemaphoreTake( xSemaphoreFuelsel, 0 ) ){
